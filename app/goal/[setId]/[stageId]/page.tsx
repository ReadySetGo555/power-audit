...
export default function GoalPage({ params }: { params: Promise<{ setId: string; stageId: string }> }) {
  ...
  if (!item) {
    ...
    return (
      <div className="app">
        <Header />
        <GoalScreen item={stub} onComplete={() => router.push("/dashboard")} onBack={() => router.push("/dashboard")} />
      </div>
    );
  }
  ...
  return (
    <div className="app">
      <Header />
      <GoalScreen item={item} onComplete={() => router.push("/dashboard")} onBack={() => router.push("/dashboard")} />
    </div>
  );
}
